...
import LaunchMonitor_pb2  # Stellen Sie sicher, dass diese Datei existiert

# --- KONFIGURATION & UUIDs ---
R10_ADDRESS = "E4:7B:DD:3F:78:D7"  # Ihre MAC Adresse

# Standard Battery Service UUID
BATTERY_CHAR_UUID = "00002a19-0000-1000-8000-00805f9b34fb"

# Device Interface Service (Control)
...
MEASUREMENT_CHAR_UUID = "6a4e3401-667b-11e3-949a-0800200c9a66"
CONTROL_POINT_CHAR_UUID = "6a4e3402-667b-11e3-949a-0800200c9a66"
...
class R10Worker(QObject):
    ...
    battery_updated = pyqtSignal(int)  # Neues Signal für Batterie (0-100)
        ...
        try:
            ...
            async with BleakClient(device) as client:
                ...
                try:
                    ...
                    if battery_data:
                        percent = int(battery_data[0])
                        self.battery_updated.emit(percent)
                        print(f"Battery: {percent}%")
                except Exception as e:
                    print(f"Battery read failed (optional): {e}")
                ...
                def notification_handler(sender, data):
                    ...
                    elif sender.uuid == BATTERY_CHAR_UUID:
                         # Manche Geräte senden Batterie-Updates via Notify
                         self.battery_updated.emit(int(data[0]))

                # Subscribe to Shot Data
                await client.start_notify(MEASUREMENT_CHAR_UUID, notification_handler)
                
                # Subscribe to Status (optional, aber gut für Debug)
                await client.start_notify(STATUS_CHAR_UUID, notification_handler)
                
                # Subscribe to Battery Notifications (falls unterstützt)
                try:
                    await client.start_notify(BATTERY_CHAR_UUID, notification_handler)
                except:
                    pass # Nicht alle Geräte unterstützen Battery Notify
...
class GolfApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Battery UI Elements
        self.battery_label = QLabel("🔋 --") # Initial text
        self.battery_label.setStyleSheet("color: #4ecca3; font-size: 16px; font-weight: bold; padding: 5px;")
        ...
        # Header Layout zusammenbauen
        header.addWidget(self.title_label)
        header.addStretch()
        
        # Batterie rechts vor Sprache
    ...
    def start_worker(self):
        ...
        self.worker.battery_updated.connect(self.update_battery) # Verbinden
        self.thread.start()

    def update_battery(self, percent):
        """Aktualisiert das Batterie-Label"""
        self.battery_label.setText(f"🔋 {percent}%")
        # Optionale Farbgebung bei kritischem Stand
        if percent < 20:
            self.battery_label.setStyleSheet("color: #e94560; font-size: 16px; font-weight: bold;")
        else:
            self.battery_label.setStyleSheet("color: #4ecca3; font-size: 16px; font-weight: bold;")
