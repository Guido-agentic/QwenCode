Here is the complete, working Python program.

### Prerequisites

1.  **Install Dependencies**:
    You need `bleak` for Bluetooth and `protobuf` for parsing the data.
    ```bash
    pip install bleak protobuf PyQt6
    ```

2.  **Create the Protocol Buffer Definition**:
    The R10 sends data in binary Protocol Buffer format. You need a file named `LaunchMonitor.proto`. Create it with the following content (based on the open-source reverse engineering of the R10):

    **File: `LaunchMonitor.proto`**
    ```protobuf
    syntax = "proto3";
    package gspro_r10.bluetooth.proto;

    message LaunchMonitor {
        float ball_speed = 1;
        float launch_angle = 2;
        float total_spin = 3;        
        int32 spin_axis = 4;         // Often reported in tenths of a degree or relative integer
        float carry_distance = 5;    // Meters
        float total_distance = 6;    // Meters
        // Note: The R10 might send more fields, but these are the core ones used by open-source adapters.
    }
    ```

3.  **Compile the Proto File**:
    Run this command in your terminal to generate the Python code:
    ```bash
    protoc --python_out=. LaunchMonitor.proto
    ```
    This creates a file named `LaunchMonitor_pb2.py`.

---

### The Complete Python Program

Save this code as `r10_monitor.py` in the same folder as your `LaunchMonitor_pb2.py`.
...
# --- KONFIGURATION & UUIDs (Corrected based on C# Source) ---
